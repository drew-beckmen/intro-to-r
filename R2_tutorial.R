...
##  2. Write loops
##  3. Write functions
##  4. Perform advanced data manipulation
...
# which stands for "concatenate" (in other words, chaining elemements together).
...
# Save the subset as iris_subset.

head(iris)

iris_subset <- subset(iris, iris$Species == "setosa") #actually not necessary to do $. 
...
# Take the following example. We are going to set an initial variable named "speed" equal to 20
# miles per hour. Each time we iterate through the loop, we will increase the speed by 5.
# As long as our speed is under 40 miles per hour, we want R to print the speed in the console.
...
# At that point, the "while" statement was no longer true (i.e., speed was over 40), so R no longer
# executed the command within the loop.
...
# console or build a break into the loop that tells R to stop iterating through the loop when speed
# is over 100. Otherwise, it will continue adding 5.
...
# to accomplish the previous code. Loops can come in handy later for many reasons. For example, you 
# may want to perform somestatistical test over many different data sets, and typing each test by hand is cumbersome. 
...
# Hint: the cor(x, y) function will return for you a correlation coefficient between two variables x and y. 
# The following code returns a correlation coefficient for mpg and wt:
...
head(mtcars)
coefficients <- c() #this line just creates an empty vector in which we will eventually store the coefficients

# We will be looking at correlations between mpg, the first column, and a series of other variables. The first of these
# other variables is disp, the 3rd column. We want to initiate a loop that correlates the 1st and 3rd columns; each time
# we iterate through the loop, it will correlate mpg with the (3+i)th column.
...
for (i in dv) {
  coefficients[i] <- cor(mtcars[, 1], columns[,i]) #could also do mtcars$mpg rather than indexing entire first column
}
...
# You could calculate it using lengthy code each time, but creating and saving a function is 
...
# Functions can take as many inputs as are necessary. A function for standard error should take
# only one input (the vector for which you will be calculating standard error).
...
mtcars$mpg_c <- center(mtcars$mpg) #added an additional column to the data table.
mean(mtcars$mpg_c) #zero!
...
# Never underestimate the power of data manipulation! Most data sets are messy or 
...
voting_data <- read.csv('social_small.csv')
head(voting_data)
...
# If sex is F, female, or Female, recode it to Female. Else recode it to Male. 
voting_data$sex <- ifelse(voting_data$sex %in% c("F", "female", "Female"), "Female", "Male")
voting_data$sex <- factor(voting_data$sex) #create factor variable with distinct levels.
summary(voting_data$sex)
...
# Writing over the existing values in the data.
voting_data$voted_2000 <- tolower(voting_data$voted_2000)
voting_data$voted_2004 <- tolower(voting_data$voted_2000)

# Don't need the %in% because there is only one equality that must be checked.
voting_data$voted_2000 <- ifelse(voting_data$voted_2000 == "yes", 1, 0)
voting_data$voted_2004 <- ifelse(voting_data$voted_2004 == "yes", 1, 0)

# Changing from numeric variable to integer format.
...
#NROW is the number of rows in the data frame
...
# The "year" column is a bit messy. We want it say only 2002 or 2004. 

voting_long$year <- parse_number(voting_long$year)

# Uh oh! An error. We need to first force it into a character variable, and then try again.

voting_long$year <- as.character(voting_long$year)
voting_long$year <- parse_number(voting_long$year)
...
USArrests_long <- melt(___, id.vars = ___,
                          variable.name = ___,
                          value.name = ___)

USArrests_summary <- ___ %>%
  group_by(___) %>%
  summarize(average = ___(___),
            se = ___(___)/sqrt(n()))
...
us_cities <- read.csv("/Users/sgg24/Downloads/us_cities.csv")
head(us_cities)  

# Create a summary table that shows the average population for each state, but filter the summary 
# table to include only states with average populations over 150,000 people per city.
# You might notice some missing values, which produce "NA" values. To exclude missing values, we
# use the "na.rm" code (meaning "remove na"). For example, to calculate a mean of a vector with
# missing values, type mean(variable, na.rm = TRUE). 

states_summary <- us_cities %>%
  ___(___) %>%
...
# Complete the following code to create a plot of the average population by state.

ggplot(states_summary, aes(x = ___, y = ___)) + 
